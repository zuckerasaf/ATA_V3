#!/usr/bin/env python3
"""
Documentation generation script for ATA_V3 project.

This script generates comprehensive HTML documentation for the entire ATA_V3 codebase
using pdoc with custom configuration and styling.
...
# Import pdoc configuration
from pdoc_config import *
...
def generate_documentation():
    ...
    # Build pdoc command with correct pdoc3 syntax
    cmd = [
        sys.executable, "-m", "pdoc",
        "--html",  # Generate HTML output
        "-o", output_dir,  # Output directory (short form)
        "-f",  # Force overwrite existing files
        "-c", "docformat='google'",  # Use Google-style docstrings
        "-c", "show_source_code=True",  # Include source code
        "-c", "show_type_annotations=True",  # Show type hints
        "-c", "show_inherited_members=True",  # Show inherited methods
        "-c", "show_signature_annotations=True",  # Show parameter types
    ]
    
    # Add optional arguments only if they have values
    if template_directory:
        cmd.extend(["--template-dir", template_directory])
    
    # Add modules to document
...
def copy_assets():
    """Copy any additional assets to the documentation directory."""
    # You can add custom CSS, images, or other assets here
    pass

def main():
    """Main function to generate documentation."""
    print("=" * 60)
    print(f"ATA_V3 Documentation Generator")
